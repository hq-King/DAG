...
class PIAD(Dataset):
    def __init__(self, run_type, setting_type, point_path, img_path, box_path, pair=2, img_size=(512, 512)):
        super().__init__()
        ...
        if self.run_type == 'train':
            ...
            self.json_file =self.read_file("Data/Seen/affordance_json_paths.txt")
            ...
            self.json_file =self.read_file("Data/Seen/affordance_json_test_paths.txt")
    ...
    def __getitem__(self, index):
        ...
        if (self.run_type=='val'):
            ...
            # point_sample_idx =  [range_[0]]


        Img = Image.open(img_path).convert('RGB') # Img: not fixed
        ...
        if(self.run_type == 'train'): # Img: [3, 224, 224], Points_List: [2, 3, 2048]
            return affordance_type, Img, Points_List, affordance_label_List, affordance_index_List, sub_box, obj_box
        else:
            return affordance_type,Img, Point, affordance_label, img_path, point_path, sub_box, obj_box
...
if __name__=='__main__':
    ...
    train_dataset = PIAD('train', "Seen", "Data/Seen/Point_Train.txt", "Data/Seen/Img_Train.txt", "Data/Seen/Box_Train.txt", 2)
    train_loader = DataLoader(train_dataset, batch_size=8, num_workers=8 ,shuffle=True, drop_last=True)
