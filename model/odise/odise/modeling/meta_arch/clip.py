...
# Local CLIP checkpoints stored under the project `ckpt/` directory.
_CLIP_PRETRAINED = {
    "ViT-L-14-336": "ckpt/clip/ViT-L-14-336px.pt",
    "ViT-L-14": "ckpt/clip/ViT-L-14.pt",
    "ViT-B-32": "ckpt/clip/ViT-B-32.pt",
}


def _clip_pretrained(name):
    return _CLIP_PRETRAINED.get(name, "openai")
...
def build_clip_text_embed(clip_model_name, labels, device="cuda", verbose=True):
    if isinstance(clip_model_name, str):
        clip, _, _ = open_clip.create_model_and_transforms(
            ...
            pretrained=_clip_pretrained(clip_model_name),
...
class ClipAdapter(nn.Module):
    def __init__(self, name="ViT-B-32", normalize=True):
        ...
        if comm.get_local_rank() == 0:
            open_clip.create_model_and_transforms(name, pretrained=_clip_pretrained(name))
        comm.synchronize()

        # checked, the same as openai original CLIP
        openai_clip, _, preprocess = open_clip.create_model_and_transforms(
            name, pretrained=_clip_pretrained(name)
        )
        super().__init__()
    ...
    def encode_text(self, text):
        text = open_clip.tokenize(text).to("cuda") # 这里的 model.device 是模型所在的设备（CPU 或 GPU）

        # 通过 CLIP 模型的 encode_text 方法提取文本特征
